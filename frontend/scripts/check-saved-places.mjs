...
class MemoryStorage {
  ...
  constructor(backing = { value: null }) {
...
// Regression P1: two distinct storage adapter objects that target the same
// underlying backend/key must still serialize their read-modify-write mutations.
const sharedBacking = { value: null };
const serviceA = new SavedPlacesService(
  new LocalSavedPlacesRepository(new MemoryStorage(sharedBacking)),
  {
    idFactory: () => "cross-home",
    now: () => "2026-08-22T00:00:00Z",
  },
);
const serviceB = new SavedPlacesService(
  new LocalSavedPlacesRepository(new MemoryStorage(sharedBacking)),
  {
