Rapporto corrente: `docs/codex-reports/2026-08-22_135300_nexo3-n3-3-voice-intent-normalization.md`
...
Aggiunti casi reali per Unicode full-width, whitespace/case normalization, punteggiatura terminale sui control intent, ambiguità, reason/confidence, raw preservation, free-text rejection e regressione invarianti Command Bus.
...
GitHub Actions `NEXO 3 Voice Validation` run `32576736422`, job `97040062658`:
- Checkout/Setup/npm ci: SUCCESS.
- Voice checker: FAILURE.
- Errore reale: `sì no` classificato `unsupported` anziché `ambiguous`.
- Conceptual validator: SKIPPED.
...
- TypeScript `--strict` interno al checker: SUCCESS.
...
- I commit di reporting successivi sono documentali; una fresh `NEXO 3 Voice Validation` sull'HEAD post-reporting viene comunque osservata nel Control Plane/handoff prima della consegna finale.

## Errori e warning
- Prima run fallita sull'ambiguità accentata, poi corretta.
...
## Problemi residui
- Serve riallineare `Fabio/FABIO_CONTROLLO.md`.
...
## Prossimo passo
Riallineare il cruscotto Fabio, osservare la validation sull'HEAD post-reporting e fare handoff a NEXO REVIEW. N3.4 non deve iniziare prima del completamento/reviewability di N3.3 secondo il Control Plane.
